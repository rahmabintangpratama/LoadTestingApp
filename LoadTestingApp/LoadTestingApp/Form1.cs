...
namespace LoadTestingApp
{
    public partial class LoadTestingApp : Form
    {
        ...
        private async Task<LoadTestResult> SendHttpRequest(HttpClient httpClient, string url, int requestNumber, int timeoutInSeconds)
        {
            ...
            {
                // Tangani timeout
                outputTextBox.AppendText($"Request {requestNumber}: Timeout - Request timed out after {timeoutInSeconds} seconds\n");
